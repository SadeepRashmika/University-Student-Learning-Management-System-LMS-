    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
                ...
                if (empty($username) || empty($email) || empty($password) || empty($full_name) || empty($student_id)) {
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            logActivity($conn, $user_id, 'create_student', "Created student: $full_name ($student_id)");
                        } else {
            ...
            case 'edit':
                ...
                $stmt = $conn->prepare("UPDATE users SET full_name = ?, email = ?, phone = ?, student_id = ?, status = ? WHERE user_id = ? AND user_type = 'student'");
                $stmt->bind_param("sssssi", $full_name, $email, $phone, $student_id, $status, $edit_user_id);
                
                if ($stmt->execute()) {
                    $message = "Student updated successfully";
                    logActivity($conn, $user_id, 'update_student', "Updated student ID: $edit_user_id");
                } else {
            ...
            case 'delete':
                ...
                if ($stmt->execute()) {
                    ...
                    logActivity($conn, $user_id, 'delete_student', "Deleted student ID: $delete_user_id");
                } else {
...
$stmt = $conn->prepare("
    SELECT u.*, COUNT(DISTINCT e.enrollment_id) as enrollment_count 
    FROM users u 
                        ...
                        <?php else: ?>
                            <?php foreach ($students as $student): ?>
                                <tr>
                                    ...
                                    <td><?php echo $student['enrollment_count']; ?> courses</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <button class="btn btn-success btn-small" onclick='openEditModal(<?php echo json_encode($student); ?>)'>
                                                ✏️ Edit
                                            </button>
                                            <button class="btn btn-danger btn-small" onclick="confirmDelete(<?php echo $student['user_id']; ?>, '<?php echo htmlspecialchars($student['full_name']); ?>')">
                                                🗑️ Delete
                                            </button>
                                        </div>
                                    </td>
